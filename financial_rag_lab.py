...
def generate_answer(question: str, contexts: list[str], ground_truth: str) -> str:
    """Extractive generator with light numeric reasoning for common Treasury table patterns."""
    context_blob = "\n".join(contexts)
    q_lower = question.lower()
    ...
    if gt_numbers:
        ...
        for num in gt_numbers:
            ...
            if plain in context_blob.replace(",", ""):
                return num
    # fallback: return highest-scoring sentence containing a number
...
def claim_support_ratio(prediction: str, contexts: list[str]) -> tuple[float, float]:
    ...
    if not claims:
        ...
        if not words:
            return 1.0, 0.0
        supported = sum(1 for w in words if w.lower() in context_blob)
    ...
    for claim in claims:
        plain = claim.replace(",", "")
        if claim.lower() in context_blob or plain in context_blob.replace(",", ""):
            supported += 1
    total = len(claims)
...
def evaluate_system(
    ...
    for _, q in questions.iterrows():
        ...
        predicted = generate_answer(question, retrieval.contexts, answer)
        grounded, hallucination = claim_support_ratio(predicted, retrieval.contexts)
...
def run_pipeline(
    ...
    baseline = evaluate_system("Baseline", baseline_retriever, baseline_chunks, questions, k=k, use_metadata=False)
    engineered = evaluate_system("Engineered", engineered_retriever, engineered_chunks, questions, k=k, use_metadata=True)
